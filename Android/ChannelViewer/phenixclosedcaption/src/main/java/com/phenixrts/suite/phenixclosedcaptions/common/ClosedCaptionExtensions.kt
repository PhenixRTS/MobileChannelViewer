...
package com.phenixrts.suite.phenixclosedcaptions.common
...
import com.phenixrts.suite.phenixclosedcaptions.models.JustificationMode
...
internal const val DEFAULT_MIME_TYPE = "application/Phenix-CC"
...
internal fun FrameLayout.drawClosedCaptions(caption: String, configuration: ClosedCaptionConfiguration) {
    ...
    caption.split("\n".toRegex()).map { it.takeIf { it.isNotBlank() } ?: " "}
        .flatMap { it.chunked(configuration.widthInCharacters) }
        .take(configuration.heightInTextLines)
        .forEach { textRow ->
